plotirfs <- function (x, plot.type = c("multiple", "single"), names = NULL, 
          main = NULL, sub = NULL, lty = NULL, lwd = NULL, col = NULL, 
          ylim = NULL, ylab = NULL, xlab = NULL, nc, mar.multi = c(0, 4, 0, 4),
          oma.multi = c(6, 4, 6, 4), adj.mtext = NA, 
          padj.mtext = NA, col.mtext = NA, ...) 
{
  ...
  plot.single <- function(x, iname, rname, ...) {
    ...
    ifelse(is.null(ylim), ylim <- range(x$lower, x$impulses, x$upper), ylim <- ylim)
    plot(xy, type = "l", ylim = range(x$lower[,rname], x$upper[,rname]), col = col[1], lty = lty[1], 
